...
function [time, reference0, position] = MOGOA(N, ArchiveMaxSize, iterM, dim, obj_no)
...
for iter=1:iterM
    ...
    c=cMax-iter*((cMax-cMin)/iterM); % Eq. (3.8) in the paper
    % c=(cMax-cMin)*cos((pi*iter)/(2*max_iter));
    % c=cMin+(cMax-cMin)*exp(-0.5*(iter/max_iter)^2);
    cc(iter)=c;
    for i=1:N
