...
%  Modified Multi-objective Grasshopper Optimization Algorithm (MOGOA) 
%  with nonlinear parameter c adaptation
%_________________________________________________________________________%
function [time, reference0, position] = NONMOGOA(N, ArchiveMaxSize, iterM, dim, obj_no)
...
cMax = 1;
cMin = 0.00004;
q = 3.5;  % Nonlinear parameter for Gaussian component

% Reference points for performance metrics
...
% 在循环之前添加数据文件初始化
data_file = 'NONMOGOA_temp_data.mat';
...
for iter = 1:iterM
    ...
    % Update positions with nonlinear c
    GrassHopperPositions_temp = zeros(N,dim);
    for i = 1:N
        % Random select c component
        r1 = rand();
        cr = 1 + floor(r1 * 4); % Select equation from 1-4
        
        % Calculate nonlinear c
        switch cr
            case 1  % Original linear
                ci = cMax - iter*((cMax - cMin)/iterM);
            case 2  % Quadratic
                ci = cMax - (cMax - cMin)*(iter/iterM)^2;
            case 3  % Exponential
                ci = cMax - (cMax - cMin)*(iter/iterM)^(1/iterM);
            case 4  % Gaussian
                exponent = -0.5*(q*iter/iterM)^2;
                ci = exp(exponent);
        end
        cc(iter)=ci;
        % Social interaction calculation
        ...
        for k = 1:2:dim
            ...
            for j = 1:N
                if i ~= j
                    ...
                    % Adaptive social component
                    s_ij = ((ub(k:k+1)' - lb(k:k+1)') .* ci/2) .* S_func(xj_xi) .* r_ij_vec;
                    S_i = S_i + s_ij;
                end
            end
            S_i_total(k:k+1) = S_i;
        ...
        % Position update
        X_new = ci * S_i_total + TargetPosition;
    ...
    % disp(['Iter ', num2str(iter), ' | Archive: ', num2str(Archive_member_no), ...
    %       ' | IGD: ', num2str(IGD_values(iter)), ' | HV: ', num2str(HV_values(iter))]);
...
% Visualization (保持原有可视化代码不变)
% figure;
% Draw_DTLZ5_3();
% hold on;
% plot3(Archive_F(:,1), Archive_F(:,2), Archive_F(:,3),'ro', 'MarkerSize', 8, 'markerfacecolor','k');
% legend('True PF','Obtained PF');
% title('Enhanced MOGOA with Nonlinear c Adaptation');
% 
% figure;
% subplot(1,2,1);
% plot(IGD_values, '-o');
% title('IGD Convergence');
% xlabel('Iteration');
% ylabel('IGD');
% 
% subplot(1,2,2); 
